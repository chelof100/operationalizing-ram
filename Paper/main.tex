...
\begin{tikzpicture}[
  ...
  box/.style={draw, rectangle, align=center,
              minimum width=5.5cm, minimum height=0.9cm, font=\small},
...
\node[box] (constraints) [below of=check]   {Check constraints $C\bigl(O(t),\,D(t)\bigr)$};

\node[box] (halt) [right=3.8cm of check]   {$A(t)=\bot$\\\textsc{halt}};
\node[box] (exec) [below left=1.3cm and 2cm of constraints]  {$A(t)=\mathrm{True}$\\\textsc{execute}};
\node[box] (deny) [below right=1.3cm and 2cm of constraints] {$A(t)=\mathrm{False}$\\\textsc{deny}};
